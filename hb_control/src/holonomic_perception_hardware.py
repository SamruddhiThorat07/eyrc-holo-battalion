...
class PoseDetector(Node):
    def __init__(self):
        ...
        self.camera_matrix = np.array(
            [[1117.854021, 0.000000, 986.329481],
             [0.000000, 1127.725651, 525.563601],
             [0.000000, 0.000000, 1.0]],
            dtype=np.float32
        )
        
        self.dist_coeffs = np.array(
            [0.023457, -0.074818, -0.000052, 0.013938, 0.0],
            dtype=np.float32
        )
       ...
       # These help detect markers of different sizes
    ...
    def image_callback(self, msg):
        ...
        try:
            ...
            #     corner_ids = [id for id in ids.flatten() if id in [1, 3, 5, 7]]
            #     self.get_logger().info(f"Corner markers found: {corner_ids}")
            # else:
            #     self.get_logger().warning("No markers detected at all!")
