...
class PoseDetector(Node):
    def __init__(self):
        ...
        # Store the latest detected position for each corner marker
        self.corner_positions = {
            1: None,  # Top-left corner
            3: None,  # Top-right corner
            5: None,  # Bottom-left corner
            7: None   # Bottom-right corner
        }
        ...
        # self.aruco_params.adaptiveThreshWinSizeMax = 100  # Increased for larger size variation
        ...
        # # Adjust perspective removal for different sizes
        ...
        # self.aruco_params.minMarkerPerimeterRate = 0.03  # Default is 0.03, lower = detect smaller
        # self.aruco_params.maxMarkerPerimeterRate = 4.0  
        
        self.detector = cv2.aruco.ArucoDetector(self.aruco_dict, self.aruco_params)
        
        self.get_logger().info('PoseDetector initialized with persistent corner tracking')
    ...
    def update_corner_positions(self, ids, corners):
        ...
        for i, id in enumerate(ids.flatten()):
            if id in self.corner_positions:
                # Store the appropriate corner point based on marker ID
                if id == 1:
                    self.corner_positions[1] = corners[i][0][0]  # top-left corner
                elif id == 3:
                    self.corner_positions[3] = corners[i][0][1]  # top-right corner
                elif id == 5: 
                    self.corner_positions[5] = corners[i][0][3]  # bottom-left corner
                elif id == 7:
                    self.corner_positions[7] = corners[i][0][2]  # bottom-right corner
    ...
    def compute_homography(self):
        ...
        # Build pixel and world point arrays in order
        pixel_points = []
        world_points = []
    ...
    def image_callback(self, msg):
        ...
        try:
            ...
            # Step 4: Update corner positions with newly detected corners
            self.update_corner_positions(ids, corners)
            
            # Step 5: Compute homography if all corners have been seen
            ...
            # Step 6-8: Process non-corner markers only if homography is available
            self.bot_poses = []
            self.crate_poses = []
            
            if ids is not None and self.H_matrix is not None:
                for i, id in enumerate(ids.flatten()):
                    if id not in [1, 3, 5, 7]:  # Skip corner markers
