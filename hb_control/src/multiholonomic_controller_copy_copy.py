    ...
    def control_cb(self):
        ...
        for bot_id in [0,1,2]:
            ...
            if state == "IDLE":
                ...
                dist,theta = self.go_to_target(bot_id,target_x,target_y,target_theta,error_threshold = self.dist_threshold[bot_id],angle_threshold = 0.1,use_target_theta = True)
                ...
                dist, theta = self.go_to_target(bot_id, target_x, target_y, target_theta, 
                                             error_threshold=280, angle_threshold=0.5, use_target_theta=True)
                
                if dist < 280 and abs(theta)<0.5:
                ...
                if self.waypoint_path[bot_id] and self.current_waypoint_index[bot_id] < len(self.waypoint_path[bot_id]):
                    ...
                    dist, theta_error = self.go_to_target(bot_id, wp_x, wp_y, target_theta,
                                                           error_threshold=80, angle_threshold=0.3, use_target_theta=True)
